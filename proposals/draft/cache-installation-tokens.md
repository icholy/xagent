...
The fix is to retain the per-installation `*ghinstallation.Transport` so its internal token cache is reused across calls. Rather than bake that into `githubserver.Server`, we put it in a **self-contained, reusable type in `internal/x/githubx`** — `githubx.AppTokenCache`. This is pure GitHub-auth machinery, and `githubx` already owns the adjacent helpers (`ParsePrivateKey`, `ParseWebHook`). `githubserver` then holds a `*githubx.AppTokenCache` and delegates to it.
...
// DefaultAppTokenCacheSize is the default number of per-installation transports
// retained by a AppTokenCache.
...
// NewAppTokenCache returns a AppTokenCache backed by the given AppsTransport (which
// authenticates as the GitHub App). maxSize bounds the number of retained
// per-installation transports; if maxSize <= 0, DefaultAppTokenCacheSize is used.
func NewAppTokenCache(app *ghinstallation.AppsTransport, maxSize int) (*AppTokenCache, error) {
    if maxSize <= 0 {
        maxSize = DefaultAppTokenCacheSize
    }
...
func (c *AppTokenCache) Token(ctx context.Context, installationID int64) (token string, expiresAt time.Time, err error) {
...
`githubserver` already parses the private key (`githubx.ParsePrivateKey`) and builds the `*ghinstallation.AppsTransport`. It now hands that transport to a `AppTokenCache` and keeps the cache instead of the bare apps transport:
...
type Server struct {
    ...
    tokens    *githubx.AppTokenCache   // was: app *ghinstallation.AppsTransport
}

func New(opts Options) (*Server, error) {
    ...
    appsTransport := ghinstallation.NewAppsTransportFromPrivateKey(http.DefaultTransport, appID, key)
    tokens, err := githubx.NewAppTokenCache(appsTransport, opts.Config.AppTokenCacheSize)
    if err != nil {
...
The max size is plumbed through `githubserver.Config` (e.g. a `AppTokenCacheSize int` field, `0` → default), so it can be tuned via the existing server-config path without code changes. A sensible default (`DefaultAppTokenCacheSize = 256`) means most deployments never set it.
...
The cache is a **bounded LRU** keyed by installation ID, with a configurable max size (`DefaultAppTokenCacheSize = 256`). When the cache is full, inserting a new installation evicts the least-recently-used one.
...
- **(b) Cached client accessor** — instead of a `Server.InstallationClient`, the accessor lives on the generic type as `AppTokenCache.Client(installationID) *github.Client`. It's available for any future *server-side* GitHub API caller (e.g. the reactions feature posting a reaction) without migrating the existing token-string consumers.
...
`s.github` is a concrete `*githubserver.Server`, so the body-only change to `CreateInstallationToken` needs no interface updates. The reactions feature (not yet in the repo) will reach tokens either through `CreateGitHubToken` (raw token) or, if it makes server-side API calls, through `AppTokenCache.Client` — both inherit the cache.

### Recommendation

1. Build `githubx.AppTokenCache` (with the bounded LRU and both `Token`/`Client` accessors) as the reusable mechanism.
2. Have `githubserver.Server` hold a `*githubx.AppTokenCache` and keep `CreateInstallationToken`'s signature unchanged (transparent option (a)). This fixes the reported waste for every current caller, including the soon-to-land reactions path, with no migration.
3. The `Client` accessor (former option (b)) ships *with* the generic type — it's free once `AppTokenCache` owns the transport — but no existing caller is migrated to it; it's there for the first server-side API caller that wants auto-rotating client auth.

## Tests

In `internal/x/githubx` (the cache is now unit-testable in isolation, with no `githubserver`/`store` dependencies):

- **Mint-count / reuse test**: stand up an `httptest.Server` that counts `POST /app/installations/{id}/access_tokens` requests and returns a token with an expiry ~1h out. Point the apps transport's `BaseURL` at it. Call `AppTokenCache.Token` N times for the same installation ID and assert exactly **one** mint reached the fake server, and that every call returned the same token and expiry. This is the regression test pinning the fix — against `master` it would see N mints.
...
- **LRU-eviction test**: construct a `AppTokenCache` with `maxSize = 1`. Mint for installation A (1 mint), mint for installation B (evicts A, 1 mint), then mint for A again and assert it **re-mints** (mint count for A goes to 2). This proves eviction drops the cached token and that re-minting is a correctness no-op.
- **`Client` accessor test**: assert `AppTokenCache.Client(id)` returns a client whose requests carry an `Authorization` header sourced from the cached transport (one mint, reused across calls).
...
- **Generic `githubx.AppTokenCache` vs. baking it into `githubserver.Server`**: the cache is pure GitHub-auth machinery with no server/store dependencies, so it belongs next to `ParsePrivateKey`/`ParseWebHook` as a reusable leaf helper. This keeps the dependency direction clean (`githubserver → githubx`), makes the cache unit-testable in isolation, and lets future non-server callers reuse it.
- **Both `Token` and `Client` accessors on the generic type**: folding the option-(b) client accessor into `AppTokenCache` (rather than the server) gives a single cached transport behind both a raw-token path and a client path — no duplicate cache, no migration of existing token-string consumers.
...
- **Config surface for max size**: expose `AppTokenCacheSize` on `githubserver.Config` only, or also as a server flag/env var? Recommend a `Config` field defaulting to `DefaultAppTokenCacheSize`; a flag can be added later if anyone needs to tune it in production.
