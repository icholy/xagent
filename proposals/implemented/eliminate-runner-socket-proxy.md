# Direct driver–control server connection: eliminate the runner socket proxy
...
In-container agents never talk to the control server directly. The runner stands up a
...
The driver and the injected `xagent` MCP server connect **directly to the control server over
...
- The runner calls `CreateTaskToken(task_id, ws.Capabilities)` on the control server (replacing
...
- `--server` / `XAGENT_SERVER` becomes the **real control server URL** instead of
...
**Network reachability & TLS.** The container must now reach the control server over the network.
The runner already knows the control server URL (its own `--server` / `XAGENT_SERVER`); it passes
...
The runner authenticates to the control server with its org-scoped `xat_` key
...
real control server URL instead of the socket URL, dropping the bind mount for **new** containers.
...
**Direct network exposure.** Every container now opens a connection to the control server rather
than to a local socket. That is the point (agents can run anywhere they can reach the
control server), but it widens the network surface: the control server must be reachable from container
...
2. **In-container control server URL defaulting (§6).** When the runner's own `--server` is a
