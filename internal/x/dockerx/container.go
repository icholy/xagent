...
// ContainerSignaler is a minimal interface for sending signals to a container.
type ContainerSignaler interface {
	ContainerKill(ctx context.Context, containerID string, signal string) error
}

// ContainerSignal sends a signal to a container without waiting for it to stop.
// It returns ErrNotRunning if the container was not running. Suitable for
// signals (e.g. SIGHUP) that should not cause the container to exit.
func ContainerSignal(ctx context.Context, client ContainerSignaler, containerID string, signal string) error {
